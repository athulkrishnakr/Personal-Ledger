...
class MyApp extends StatefulWidget {
  MyApp({Key? key}) : super(key: key) {
    // TransactionDB v = TransactionDB.init();
    // dynamic f = v.DB;
  }
...
class _MyAppState extends State<MyApp> {
  ...
  void RetrieveData() async {
    // dynamic sd = WDb.delete();
    List<Map<String, Object?>> list = await WDb.read();
    a = list.map((json) => wallet.fromJson(json)).toList();
    setState(() {
      for (int k = 0; k < a.length; k++) {
        walletname.insert(0, a[k].walletname);
        amount.insert(0, int.parse(a[k].amount));
      }
    });
  }
